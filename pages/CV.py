...
if 'id' not in st.session_state:
    ...
    def run_CV():
        ...
        with st.toast('Simulation started'):
            ...
            CV_par_obj = utils_CV.read_CV_parameters(CV_par, dev_par[zimt_device_parameters])

            # Run the CV script
            result, message = CV_exp.run_CV_simu(zimt_device_parameters, session_path, CV_par_obj["tVGFile"], CV_par_obj["freq"],
                                                               CV_par_obj["Vmin"],CV_par_obj["Vmax"],CV_par_obj["delV"],
                                                               CV_par_obj["Vstep"],CV_par_obj["G_frac"],True, tj_name=CV_par_obj['tJFile'])
    ...
    def upload_devpar_file(uploaded_file, uploaded_files):
        ...
        utils_gen.upload_single_file_to_folder(uploaded_file, session_path) # Save the simulation setup file
        utils_gen.upload_multiple_files_to_folder(uploaded_files, session_path) # Save the layer parameter files
